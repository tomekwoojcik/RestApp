...
export function LoginPageProvider({ children }: propsModel) {
  ...
  const toggleValue = () => {
    if (filterLogin.length === 1) {
      const errors = [...handleError, "The user is being recovered, please try again later or contact your administrator."];
      setHandleError(errors);
      return;
    }
    if (state.loginInput.length < 5) {
      const errors = [...handleError, "The user is not correct, please try again later or contact your administrator."];
      setHandleError(errors);
      return;
    }
    setToggleLogin(true);
