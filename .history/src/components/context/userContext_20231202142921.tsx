...
interface UserModel {
  ...
  open: boolean;
  setHandleNav: React.Dispatch<React.SetStateAction<navModel>>;
}
...
export function UserProvider({ children }: propsModel) {
  ...
  const[handleNav, setHandleNav] = useState<navModel>({routes:`${}`, toggleEffect:false})
  ...
  useEffect(() => {navigate(handleNav.routes)}, [handleNav.toggleEffect]);
