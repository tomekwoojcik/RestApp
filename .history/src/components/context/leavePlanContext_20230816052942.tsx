...
import { initState, REDUCER_ACTION_TYPE, reducer } from "../../hooks/leavePlanHooks";
...
interface LeavePlanContextModel {
  handleDay: (type: REDUCER_ACTION_TYPE, e: ChangeEvent<HTMLInputElement>) => void;
  leaveArr: string[];
...
export function LeavePlanProvider({ children }: propsModel) {
  ...
  const handleDay = (type: REDUCER_ACTION_TYPE, e:ChangeEvent<HTMLInputElement>) => {
    dispatch({
      type: type,
      handle:e
    })
