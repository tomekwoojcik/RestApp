...
export default function PersistentDrawerLeft() {
  ...
  const { data, userButtons, supervisorButtons, userRoleArr, navigate } = useContext(
    UserContext,
  );
        ...
        <List>
          {userButtons.map(
            (button:MenuButtonModel) => (
              <ListItem onClick={ navigate(button.routes)} key={button.buttonName} disablePadding>
                <ListItemButton>
                  <ListItemIcon>
                    {button.key % 2 === 0 ? <InboxIcon /> : <MailIcon />}
                  </ListItemIcon>
