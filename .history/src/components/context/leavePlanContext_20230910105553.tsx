  ...
  const handleActionLeave = (e: any) => {
    switch (e.name) {
      case "Confirm": {
        ...
        const changeStatusObj = item.holidayWorkerApprovalStatus = "leave confirm";

        if (state.booleanValue == true) {
          removeObjOfDatabase(leaveConfirmData, leaveData, changeStatusObj, e);
        }
        setCount();
        break;
      }
      case "Cancel": {
        const item = searchLeaveObj({ leaveData, e });
        const changeStatusObj = item.holidayWorkerApprovalStatus = "leave cancel";
        removeObjOfDatabase(leaveCancelData, leaveData, changeStatusObj, e);
        setCount();
        break;
      }
    }
