...
export function SupervisorProvider({ children }: propsModel) {
  ...
  const supervisorEmployeeLeaveObjConfirm = new Data("supervisorEmployeeLeaveObjConfirm");
  ...
  const handleMenuAction = (buttonName:string, leaveId:string) => {
    switch (buttonName) {
      case "confirm": {
        const [leaveObj] = leaveData.getData().filter((leaveObj: leaveObjModel) => {
          leaveObj.leaveId == leaveId
        })
  ...
  console.log(getEmployeeLeaveObj);
