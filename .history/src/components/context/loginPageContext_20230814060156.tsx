...
export function LoginPageProvider({ children }: propsModel) {
  ...
  const filterLogin = dataEl.filter(
    (el: { userId: string; }) => el.userId === state.loginInput,
  );
  ...
  const toggleValue = () => {
    if (filterLogin.length === 1) {
      handleError("The user is being recovered, please try again later or contact your administrator.")
      return
    }
    if (state.loginInput.length < 5) {
      handleError("The user is not correct, please try again later or contact your administrator.")
      return
    }
  ...
  console.log('state:', state.errorArr)
  ...
  const loginHandle = async (): Promise<void> => {
    try {
      ...
      const { accessToken }: ResponseModel = res;
      const { user }: userModel["user"] = res;
