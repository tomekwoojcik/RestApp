...
export function LoginPageProvider({ children }: propsModel) {
  ...
  const loginHandle = async (): Promise<void> => {
    ...
    try {
      const userAccess = await userLoginApi(state.loginInput, state.passwordInput);
      localStorage.setItem("token", userAccess.data.accessToken);
      getLoggedUser(userAccess.data);
      const foo: any = getUsersResource(userAccess.data);
      dispatch({
