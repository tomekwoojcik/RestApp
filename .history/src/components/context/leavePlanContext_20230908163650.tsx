...
export function LeavePlanProvider({ children }: propsModel) {
  ...
  const firstDayLeave = destructObj(state.firstDay)?.toString().slice(0, 15);
  const lastDayLeave = destructObj(state.lastDay)?.toString().slice(0, 15);
  ...
  const leavePlanHandle = () => {
    ...
    const lastDayOfLeave = new Date(lastDayLeave == undefined ? todayDate : lastDayLeave);
    ...
    if (firstDayLeave?.slice(0, 2) == "sun" || firstDayLeave?.slice(0, 2) == "sat" || lastDayLeave?.slice(0, 2) == "sun" || lastDayLeave?.slice(0, 2) == "sat") {
      handleWarnMess(
        todayDate,
        "The leave day cannot be Saturday or Sunday."
      )
    }
