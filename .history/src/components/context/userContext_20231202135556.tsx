...
interface UserModel {
  ...
  navigate: any;
  userRoleArr: string[];
  setRoutesNavigate: React.Dispatch<React.SetStateAction<string>>
}
...
export function UserProvider({ children }: propsModel) {
  ...
  const [routesNavigate, setRoutesNavigate] = useState<string>("/login");
  ...
  useEffect(() => {navigate(routesNavigate)}, [routesNavigate]);
