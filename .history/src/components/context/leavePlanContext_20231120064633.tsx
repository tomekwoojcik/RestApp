...
export function LeavePlanProvider({ children }: propsModel) {
  ...
  const handleActionLeave = (e: any) => {
    switch (e.name) {
      case "Confirm": {
        ...
        item.supervisorApprovalStatus = "leave request pending";
  ...
  const handleActionForm = (obj: leaveObjModel): void => {
    ...
    const filterData: leaveObjModel[] = getData.filter((leaveObj: leaveObjModel) => obj.leaveId == leaveObj.leaveId)
    if (filterData[0].leaveId != obj.leaveId) {
