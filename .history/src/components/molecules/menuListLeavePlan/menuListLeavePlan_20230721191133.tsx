...
function MenuListLeavePlan({ leaveId }) {
  ...
  const handleOpen = e => setMenuBooleanList(e.currentTarget);
  const handleClose = e => {
  ...
  return (
    <div>
      ...
      >
        {menuListArr.map(name => <MenuItem key={Math.random()} onClick={event => handleClose({ event, name, leaveId })}>{name}</MenuItem>)}
