...
function LeavePlanDetails() {
  ...
  return (
    <div>
      <ul>
        {dataRender.map((el:any) => <LeavePlanDetailsRow key={Math.random()} details={el} personId={''} kindLeave={''} startDateOfLeave={''} endDateOfLeave={''} replacementPerson={''} leaveId={''} holidayWorkerApprovalStatus={''} />)}
      </ul>
      {warnMess.length != 0 ? (
        <div>
          <h3>Warning List</h3>
