...
export const MessageProvider = ({ children }: propsModel) => {
  ...
  const getFormElements = (
    ...
    const handleMessage = () => {
        ...
        const messageObj = new Messages(state.messageReason, getUserData.id, userObj.id);
        messageObj.sentMessage(state.messageText);
        const messObj = MessageObject(state.messageText);
    }
