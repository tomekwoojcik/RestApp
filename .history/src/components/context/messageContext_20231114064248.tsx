...
interface MessageObjectModel {
  ...
  sentMessage: (messageObj: MessageObject) => void;
}
...
export const MessageProvider = ({ children }: propsModel) => {
  ...
  const setMessageInRoom = (messageObj: MessageObjectModel): void => {
    dispatch({
      ...
      dispatch({
          type: REDUCER_ACTION_TYPE.SET_CLICK_MESSAGE_COUNTER,
          payload: ++state.clickMessage 
      })
  };

    useEffect(() => {
        dispatch({
            type: REDUCER_ACTION_TYPE.GET_MESSAGE_ARR,
            payload:state.messageObj.messagesArr
      })
  }, [state.clickMessage]);

  console.log(state.getMessageArr);

  return (
    <MessageContext.Provider
      value={{ state, getFormElements, handleMessage, setMessageInRoom }}
    >
      {children}
    </MessageContext.Provider>
  );
